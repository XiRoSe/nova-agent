        ...
        if (fs.existsSync(messagesDir)) {
          ...
          for (const file of messageFiles) {
            ...
            try {
              ...
              } else if (data.type === 'image' && data.chatJid && data.imageBase64 && data.mimeType) {
                // Authorization: verify this group can send to this chatJid
                const targetGroup = registeredGroups[data.chatJid];
                if (
                  isMain ||
                  ...
                  await deps.sendImage(data.chatJid, data.imageBase64, data.mimeType, data.caption);
                  logger.info(
                    { chatJid: data.chatJid, sourceGroup },
                    'IPC image sent',
                  );
              ...
              } else if (data.type === 'image_url' && data.chatJid && data.imageUrl) {
                // Authorization: verify this group can send to this chatJid
                const targetGroup = registeredGroups[data.chatJid];
                if (
                  isMain ||
                  ...
                  await deps.sendImageUrl(data.chatJid, data.imageUrl, data.caption);
                  logger.info(
                    { chatJid: data.chatJid, sourceGroup },
                    'IPC image URL sent',
                  );
